...
interface UpdatePayload {
  ...
  businessDescription: string;
  email: string;
  packageDetails: Record<string, { photos: number; locations: number; price: number }>;
}
...
export const updatePhotographer = createAsyncThunk(
  ...
  async (payload: UpdatePayload, { rejectWithValue }) => {
    try {
      const response = await photographerClient.put(`/photographer/${payload.id}/update`, payload);
      return response.data;
    } catch (error) {
